	...
	public function store(Request $request, $req = true) {
		...
		foreach ($fileData as $n => $rows) {
			...
			foreach ($rows as $i => $row) {
				...
				if (canvastack_string_contained($fieldname, 'username')) $fieldvalue = strtolower($row);
				if (canvastack_string_contained($fieldname, 'fullname') || canvastack_string_contained($fieldname, 'alias') || canvastack_string_contained($fieldname, 'info') || canvastack_string_contained($fieldname, 'maps')) $fieldvalue = ucwords($row);
				
				if (canvastack_string_contained($fieldname, 'role')) $contentFile['roles'][$n]['role']  = $fieldvalue;
				...
				if (canvastack_string_contained($fieldname, 'info'))  $contentFile['roles'][$n]['info']  = $fieldvalue;
				if (canvastack_string_contained($fieldname, 'alias')) $contentFile['roles'][$n]['alias'] = $fieldvalue;
