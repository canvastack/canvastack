...
class ExtransloadController extends Controller {
	...
	public function edit($id) {
		...
		$ajaxURL = canvastack_get_ajax_urli('canvastackHostProcess');
		$this->form->draw(canvastack_script("
jQuery(document).ready(function() {
	$('form[name=\"{$this->form->identity}\"]').on('submit', function(e) {
		var submitButtonBox = $('div.canvastack-action-box>input.btn');
		submitButtonBox.css({
			'width': '220px',
			'text-align': 'left',
			'text-indent': '10px'
		...
		$.ajax({
			...
			complete : function() {
				...
				$('div.canvastack-action-box').append(submitButtonBox);
	...
	public function synconnections(string $source_field, string $target_field, $selected = null) {
		...
		$ajaxURL              = canvastack_get_ajax_urli('canvastackHostConn', $connections);
