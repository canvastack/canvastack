...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        // Use auto-detection instead of hard-coded foreign keys
    ...
    public function store(Request $request)
    {
        ...
        // CRITICAL: Validate request BEFORE processing
        ...
        return redirect("system/accounts/user/{$this->stored_id}/edit")->with('success', 'User created successfully');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Adjust validation for updates
        $updateValidations = $this->validations;
        
        // Remove password requirement if not provided
        if (null === $request->password || empty($request->password)) {
            unset($updateValidations['password']);
            $request->offsetUnset('password');
        }
        
        // Allow same email for current user
        if ($this->model_data->email === $request->email) {
            $updateValidations['email'] = 'required|email';
        }
        
        // Allow same username for current user
        if ($this->model_data->username === $request->username) {
            $updateValidations['username'] = 'required';
        }
        
        $request->validate($updateValidations);

        $this->set_data_before_post($request, __FUNCTION__);
        $this->update_data($request, $id, false);
        $this->set_data_after_post($this->group_id, $id);

        return redirect("system/accounts/user/{$id}/edit")->with('success', 'User updated successfully');
    }
    ...
    private function set_data_after_post($data, $id = false)
    {
        try {
            ...
            if (false !== $id) {
                // Update case - find existing relationship and update, or create new
                ...
                if ($existingGroup) {
                    ...
                    // Delete old relationships for this user to avoid duplicates
                    $user_group->where('user_id', intval($id))->delete();
