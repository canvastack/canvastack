...
class UserController extends Controller {
	...
	public function __construct($import = false) {
		...
		$maxFileSizeMB = intval($maxFileSizeBytes / 1048576); // Convert to MB for setImageElements
		...
		// Set image elements for photo with thumbnail generation
		$this->setImageElements('photo', $maxFileSizeMB, true, [150, 150]);
	}
	...
	private function sendEmail($to, $subject, $title, $message, $cc = []) {
		...
		try {
			...
			$email = new \Canvastack\Canvastack\Library\Components\Messages\Email\Objects();
	...
	public function store(Request $request) {
		$this->get_session();
		if (!empty($request->canvastackImportProcess)) return $this->storeFromImports($request);
	...
	private function set_data_after_post($data, $id = false) {
		...
		if (false !== $id) {
			$userGroup = canvastack_query_get_id($user_group, ['user_id' => intval($id)]);
			if (!is_null($userGroup)) {
				canvastack_update($user_group->find($userGroup->id), $request, true);
			} else {
