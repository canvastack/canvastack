...
use Canvastack\Canvastack\Library\Components\Charts\Canvas\Builder;
...
class Objects extends Charts {
	...
	protected function setParams($type, $source, $fieldsets = [], $format, $category = null, $group = null, $order = null, $options = []) {
		$this->sourceIdentity = canvastack_clean_strings("CanvaStack_{$this->chartLibrary}_" . $source . '_' . canvastack_random_strings(50, false));
	...
	protected $post = [];
	protected $chartPostData = 'canvastackChartData';
	public function process($post) {
		if (!empty($_GET['canvastackChartDataFilter'])) {
			$postFilter          = [];
			$chartIdentityFilter = 'postFromTable' . $_GET['canvastackChartDataFilter'];
			$chartIdentityCode   = $_GET['canvastackChartData'];
