...
trait Scripts {
	...
	protected function filterTableInjectionScript($data) {
		...
		$submitFilterButton = str_replace('_CanvaStackFILTERForm', '_submitFilterButton', "{$formIdentity}");
		$urli               = $this->chartURI[$chartIdentity[$formIdentity]['code']] . '&canvastackChartDataFilter=' . $chartIDString . '&canvastackChartData=' . $chartIDCode;
		
		unset($chartIdentity[$formIdentity]['source']);
		unset($chartIdentity[$formIdentity]['code']);
		unset($chartIdentity[$formIdentity]['string']);
		
		$data['datatables']['source'] = 'canvastackChartDataFilter::' . canvastack_encrypt($data['datatables']['source']);
