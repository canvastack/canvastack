...
trait DataModel {
		...
		if (!empty($data)) {
			...
			if (!empty($sourceData->format)) {
				...
				foreach ($sourceData->fields as $fieldset) {
					...
					if (canvastack_string_contained($fieldset, '::')) {
				...
				if (canvastack_string_contained($sourceData->source, 'canvastackChartDataFilter::')) {
					$sourceTable = canvastack_decrypt(str_replace('canvastackChartDataFilter::', '', $sourceData->source));
				}
				...
				$queryData          = canvastack_query($sql, 'SELECT', $this->connection);
	...
	private static function manipulate($type = 'column', $source, $parameters, $category) {
		...
		foreach ($chartData['data'] as $name => $data) {
			...
			if (canvastack_string_contained($chartType, 'line')) {
				$buffers['series'][] = [
					'name'      => $name,
					'data'      => $data,
					'type'      => $chartType,
