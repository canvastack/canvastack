...
trait RouteInfo
{
    ...
    private function get_pageinfo()
    {
        ...
        // Environment-aware logging for page info extraction
        if (app()->environment(['local', 'testing'])) {
            SafeLogger::debug('RouteInfo: Extracting page information', [
                'controller_path' => $controller_path,
                'route_name' => $this->currentRoute->getName()
            ]);
        }
        ...
        // Environment-aware logging for extracted page info
        if (app()->environment(['local', 'testing'])) {
            SafeLogger::debug('RouteInfo: Page information extracted', [
                'page_info' => $this->pageInfo,
                'controller_name' => $this->controllerName,
                'controller_path_name' => $this->controllerPathName
            ]);
        }
    }
    ...
    public function routeInfo()
    {
        ...
        if (strpos(php_sapi_name(), 'cli') === false) {
            // Environment-aware logging for route info processing start
            if (app()->environment(['local', 'testing'])) {
                SafeLogger::debug('RouteInfo: Starting route info processing', [
            ...
            // Environment-aware logging for route info completion
            if (app()->environment(['local', 'testing'])) {
                SafeLogger::debug('RouteInfo: Route info processing completed', [
                    'current_path' => $this->currentRoute->getName(),
                    'module_name' => $this->controllerName,
                    'page_info' => $this->pageInfo,
                    'action_pages_count' => count($action_page['action_page'] ?? [])
                ]);
            }

            $this->setDataValues('route_info', (object) array_merge($routeInfo, $action_page));
        } else {
            // Environment-aware logging for CLI request
            if (app()->environment(['local', 'testing'])) {
                SafeLogger::debug('RouteInfo: Skipping route info processing for CLI request', [
                    'sapi_name' => php_sapi_name()
                ]);
            }
        }
    ...
    private function routeReplaceURL($from, $to)
    {
        ...
        if ('destroy' !== $to) {
            ...
            $routeURI = $routeUri;
            $routeUri = explode('/', canvastack_current_url());
