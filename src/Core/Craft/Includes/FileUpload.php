...
trait FileUpload
{
    ...
    public function uploadFiles($upload_path, $request, $file_data = [])
    {
        ...
        // Environment-aware logging for upload initialization
        if (app()->environment(['local', 'testing'])) {
            SafeLogger::debug('FileUpload: Starting file upload process', [
                'upload_path' => $upload_path,
                'base_upload_dir' => $baseUploadDir,
                'upload_disk' => $uploadDisk,
                'file_data_count' => is_array($file_data) ? count($file_data) : 0
            ]);
        }

        // Cek traversal: Tidak boleh mengandung '../' atau '..'
        if (strpos($upload_path, '../') !== false || strpos($upload_path, '..\\') !== false) {
            // Environment-aware logging for security violation
            if (app()->environment(['local', 'testing'])) {
                SafeLogger::warning('FileUpload: Path traversal attempt detected', [
                    'attempted_path' => $upload_path,
        ...
        // Environment-aware logging for sanitized path
        if (app()->environment(['local', 'testing'])) {
            SafeLogger::debug('FileUpload: Path sanitized', [
                'sanitized_path' => $upload_path
            ]);
        }
        ...
        if (is_array($file_data)) {
            foreach ($file_data as $file_name => $config) {
                if ($request->hasFile($file_name)) {
                    ...
                    if (app()->environment(['local', 'testing'])) {
                        SafeLogger::debug('FileUpload: Processing file', [
                            'file_name' => $file_name,
                            'original_name' => $file->getClientOriginalName(),
                            'file_size' => $file->getSize(),
                            'mime_type' => $file->getMimeType(),
                            'has_validation' => isset($config['file_validation'])
                        ]);
                    }
                    ...
                    if (isset($config['file_validation'])) {
                        ...
                        // Environment-aware logging for validation
                        if (app()->environment(['local', 'testing'])) {
                            SafeLogger::debug('FileUpload: File validation passed', [
                                'file_name' => $file_name,
                                'validation_rules' => $config['file_validation']
                            ]);
                        }
                    }
                    ...
                    // Environment-aware logging for successful upload
                    if (app()->environment(['local', 'testing'])) {
                        SafeLogger::debug('FileUpload: File uploaded successfully', [
                            'file_name' => $file_name,
                    ...
                    if (isset($config['thumb_name']) && $config['file_type'] === 'image') {
                        // Environment-aware logging for thumbnail processing
                        if (app()->environment(['local', 'testing'])) {
                            SafeLogger::debug('FileUpload: Processing thumbnail', [
                                'file_name' => $file_name,
                                'thumb_name' => $config['thumb_name'],
                        ...
                        // Environment-aware logging for thumbnail success
                        if (app()->environment(['local', 'testing'])) {
                            SafeLogger::debug('FileUpload: Thumbnail created successfully', [
                                'file_name' => $file_name,
        ...
        if (empty($this->getFileUploads)) {
            // Environment-aware logging for no uploads
            if (app()->environment(['local', 'testing'])) {
                SafeLogger::warning('FileUpload: No files were uploaded', [
                    'redirect_route' => str_replace('store', 'create', current_route())
                ]);
            }
        ...
        // Environment-aware logging for upload completion
        if (app()->environment(['local', 'testing'])) {
            SafeLogger::debug('FileUpload: Upload process completed', [
                'total_files_uploaded' => count($this->getFileUploads),
                'uploaded_files' => array_keys($this->getFileUploads)
            ]);
        }
